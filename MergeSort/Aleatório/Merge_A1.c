...
int main() {
    ...
    // Preenche o vetor com 1000 valores em ordem aleatória
    srand(time(NULL));
    for (vAux = 0; vAux < TAM; vAux++) {
        vVetor[vAux] = rand() % TAM; // Gera um número aleatório entre 0 e TAM - 1
    }

    printf("Valores originais no vetor (em ordem aleatória):\n");
    imprimir(vVetor, TAM);
    ...
    printf("\n\nValores ordenados pelo Merge Sort (em ordem aleatória):\n");
    imprimir(vVetor, TAM);
